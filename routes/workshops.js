...
router.post('/txn', (req, res, next) => {
  ...
  firestore.collection('workshopOrders').doc(body['ORDER_ID']).get()
    .then(docSnapshot => {
      const params = {};
      ...
      params['CALLBACK_URL'] = baseURL + '/workshops/response';
...
router.post('/response', async (req, res, next) => {
  ...
  if (checksum_lib.verifychecksum(paramList, paytmConfig.PAYTM_MERCHANT_KEY, paramList.CHECKSUMHASH)) {
    if (paramList.STATUS === 'TXN_SUCCESS') {
      const orderRef = await firestore.collection('workshopOrders').doc(paramList.ORDERID).get();
      const orderData = orderRef.data();
      firestore.collection('workshopOrders').doc(paramList.ORDERID).update({
        ...orderData,
        paymentStatus: 'paid'
      })
